...
#if sunny need sunglasses, if rainy need raincoat
-
...
function leaving_the_house {
    always_need "phone,wallet, and keys"
    if "rainy" then bring "raincoat"
    otherwise ___
...
leaving_the_house <- function(weather) {print("Need phone, wallet, and keys")
  if (weather== "rainy") {
    print("Also, need rain jacket")}
  if (weather== "sunny") {
    print("Also need sunglasses")
  }
}

#looked up how to combine messages into single print, ended up using if and paste
#final function is below

leaving_the_house <- function(weather,day) {
  x<-"Need phone, wallet, and keys."
    if (weather== "rainy") {
      x <- paste(x, "Also need raincoat.")
      }
    if (weather== "sunny") {
      x <- paste(x,"Also need sunglasses.")
      }
    if (day== "weekday") {
      x <- paste(x, "Need backpack too.")
     }
    if (day== "weekend") {
      x<- paste(x, "Don't need backpack.")
      }
  return(x)
    }
```
...
leaving_the_house("rainy")

leaving_the_house("sunny")

leaving_the_house()

leaving_the_house("rainy","weekday")

leaving_the_house("sunny","weekday")

leaving_the_house("rainy","weekend")

leaving_the_house("sunny","weekend")
...
today <- leaving_the_house("sunny","weekday")
today

tomorrow <- leaving_the_house("rainy","weekend")
tomorrow
```
...
comma1 <- function(x) {
  y<-
    round(x)
  z<-formatC(y,big.mark = ",", format = "f", digits = 0)
  paste0(z,collapse = "; ")
}
comma1(comma_vals)
```
...
pct(c(0.59,0.02,0.96,0.123,0.005))
pct(c(0.05, 0.1, 0.512, 0.678))

currency<- scales::label_currency()
currency(c(100, 110.70, 823, 1123))

comma<- scales::label_comma()
comma(c(2345, 10123.4, 999, 1205))
```
